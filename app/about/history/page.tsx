    ...
    <main className="bg-gradient-to-b from-slate-950 via-[#3d080f] to-black text-white min-h-screen py-16">
      <div className="max-w-5xl mx-auto px-4 space-y-12">
        <header className="space-y-4 text-center">
          <p className="text-sm uppercase tracking-[0.4em] text-red-200">Our Story</p>
          <h1 className="text-3xl md:text-5xl font-extrabold">CITAM Kisumu Journey</h1>
          <p className="text-gray-300 max-w-3xl mx-auto">
            From a small prayer gathering in Nairobi to a vibrant community across the nations, learn how Christ has led CITAM to impact Kisumu and beyond.
          </p>
        </header>

        <section className="relative">
          <div className="absolute left-5 md:left-1/2 transform -translate-x-1/2 h-full border-l border-white/20" aria-hidden />
          <div className="space-y-12">
            {timeline.map((item, index) => (
              <article
                key={item.year}
                className={`relative flex flex-col md:flex-row ${index % 2 === 0 ? "md:flex-row-reverse" : ""} gap-6 items-center`}
              >
                <div className="md:w-1/2 space-y-3">
                  <p className="text-xs uppercase tracking-[0.4em] text-red-200">{item.year}</p>
                ...
                <span className="absolute left-5 md:left-1/2 transform -translate-x-1/2 w-3 h-3 rounded-full bg-red-500 border-4 border-black" aria-hidden />
              </article>
            ))}
          </div>
        </section>
        ...
        <div className="bg-white/10 border border-white/10 rounded-3xl p-8 text-center space-y-4">
          <h3 className="text-2xl font-semibold">Join the ongoing story</h3>
          <p className="text-gray-200">
            We believe God is writing new chapters in Kisumu and beyond. Be part of this move by serving, giving, and gathering in community each week.
          </p>
          <Link
            href="/"
